...
function writeContents(writePath, file, cb) {
  ...
  function written(err, fd) {
    ...
    function applyStatFd(err, st) {
      if (err) {
        return complete2(err);
      }

      var mode = getMode(st, file.stat);
      if(mode == null) {
    ...
    function applyStatPath(err, st) {
      ...
      fs.chmod(writePath, mode, function(error) {
        if (error) {
    ...
    if (typeof fd === 'number') {
      return fs.fstat(fd, applyStatFd);
    }

    fs.lstat(writePath, applyStatPath);
  }
