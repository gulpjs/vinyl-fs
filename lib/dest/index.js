...
function dest(outFolder, opt) {
  ...
  if (typeof options.dirMode === 'string') {
    options.dirMode = parseInt(options.dirMode, 8);
  }

  var cwd = path.resolve(options.cwd);
  var dirMode = (options.dirMode || processMode);
  ...
  function saveFile (file, enc, cb) {
    ...
    mkdirp(writeFolder, dirMode, function(err){
      if (err) {
        cb(err);
      } else {
