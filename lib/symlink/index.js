'use strict';

var through2 = require('through2');
var fs = require('graceful-fs');
var path = require('path');
var valueOrFunction = require('value-or-function');

var prepareWrite = require('../prepare-write');
var defaultValue = require('../default-value');

var boolean = valueOrFunction.boolean;
var string = valueOrFunction.string;

function symlink(outFolder, opt) {
  if (!opt) {
    opt = {};
  }

  function linkFile(file, enc, cb) {
    var rel = defaultValue(false, boolean(opt.relative, file));
    var srcPath = rel
      ? path.relative(absolute(outFolder, opt, file), file.path)
      : file.path;
    var symType = (file.isDirectory() ? 'dir' : 'file');
    prepareWrite(outFolder, file, opt, function(err) {
      if (err) {
        return cb(err);
      }

      fs.symlink(srcPath, file.path, symType, function(err) {
        if (err && err.code !== 'EEXIST') {
          return cb(err);
        }
        cb(null, file);
      });
    });
  }

  var stream = through2.obj(opt, linkFile);
  // TODO: option for either backpressure or lossy
  stream.resume();
  return stream;
}

function absolute(outFolder, opt, file) {
  var cwd = defaultValue(process.cwd(), string(opt.cwd, file));
  return path.resolve(cwd, string(outFolder, file));
}

module.exports = symlink;
